...
[정리 표지](../types/auto_cleanup.md) 역시 쌍대성에 의해 보존된다. 다음 타입은...
...
선택 측에서는 `.cancel`이 자동으로 선택해도 안전한 분지라는 약속을 하고 있기 때문에, 분기 측에서도 같은 표지를 사용해 런타임에 사용자가 (`.case`문 안에 별표를 추가해서) 정리 분지를 등록하고 자동 정리에서 선택될 수 있도록 해야 한다.
